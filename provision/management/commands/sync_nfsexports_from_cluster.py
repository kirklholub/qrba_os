...
class Command(BaseCommand):
    help = "synchronizes quotas from cluster to system objects"
    ...
    def handle(self, *args, **options):
        ...
        if cluster.count() == 1:
            ...
            print("now: " + str(now))
            print("calling sync_nfs_exports_from_cluster ( " + str(cluster.name) + ") at " + str(now))
            activity = cluster.sync_nfs_exports_from_cluster()
